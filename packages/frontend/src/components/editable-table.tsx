...
type TableState = {
  ...
  subscriptions: Map<
    string,
    Set<{ callback: (payload: unknown) => void; immediate: boolean }>
  >;
  rowOrder: string[];
  columnOrder: string[];
...
type ActionHandler<P> = (
  state: TableState,
  payload: P,
  invalidate: (tag: string, payload?: unknown) => void,
) => void;
...
const actionHandlers: {
  ...
  SET_CELL_ANNOTATION: (
    ...
    const annotations: Annotations = state.data
      .get(row)
      .cells.get(column).annotations;
...
const handleAction = (
  ...
  actionHandlers[event.type](state, event.payload as any, (tag, payload) =>
...
const createInvalidableHook = <A extends unknown[], T>(
  ...
  return (...args) => {
    ...
    useEffect(() => {
      ...
      names(...args).forEach(tag => subscribe(tag, listenerRef.current));
...
const useRowState = createInvalidableHook(
  (rowKey: string) => [`row-${rowKey}`],
  (state, rowKey) => state.data.get(rowKey),
);
const useCellState = createInvalidableHook(
  (rowKey: string, columnKey: string) => [
    'cell',
    `cell-${rowKey}-${columnKey}`,
  ],
  (state, rowKey, columnKey) => state.data.get(rowKey).cells.get(columnKey),
);

const useColumnState = createInvalidableHook(
...
const TableProvider = ({ children, ...props }: PropsWithChildren<Props>) => {
  ...
  const subscribe = useCallback(
    (tag: string, callback: (payload: unknown) => void, immediate = false) => {
          ...
          setTimeout(
            () =>
              resolve(
                columnType.validate(
                  value,
                  createRowApiObject(ref.current, dispatch, row),
                ),
              ),
            0,
          );
...
const StatusIndicator = (props: StatusIndicatorProps) => {
  ...
  const getIcon = (type: AnnotationType) =>
    ({
      ...
      success: (props: any) => (
        <CheckCircle className="text-green-500" {...props} />
      ),
        ...
        <Dropdown
          options={[
            ...
            ...props.rowActions.map((action: RowAction) => ({
              ...
              onSelect: () => {
                action.execute(
                  createRowApiObject(ref.current, dispatch, row.key),
                );
              },
            })),
          ]}
...
export const EditableTableInner = forwardRef((props: Props, ref) => {
  ...
  useImperativeHandle(
    ...
    (): TableRef => {
      return {
        getRow: (key: string) =>
          createRowApiObject(tableRef.current, dispatch, key),
        setData: (data: string[][]) => {
          dispatch({
            type: 'CLEAR_CELLS',
            payload: null,
