    ...
    case 'call':
      expr.arguments.forEach(argExpr => {
        ...
        if (
          ...
          throw new Error(
            'Function call arguments must be either a number or a string! Got ' +
              typeof arg,
          );
        }
...
const functions: Record<
  string,
  (event: AuditEvent, ...rest: (string | number | boolean)[]) => React.ReactNode
> = {
  ...
  value(_, value) {
    return (
      <span className="mx-1 rounded-sm bg-gray-200 px-1 font-mono text-xs shadow-sm">{`${value}`}</span>
    );
  },
};
  ...
  if (!format) {
    return (
      <>
        ...
        <span className="mx-1 rounded-sm bg-gray-200 px-1 font-mono text-xs shadow-sm">{`${event.action}`}</span>
        .
      </>
    );
  }
      ...
      {parts.map(part => {
        ...
        if (part.type === 'expression') {
          try {
            ...
            console.error(
              `Error while evaluating expression "${formatExpression(part.value)}":`,
              err,
            );
            return '<Error>';
          }
...
const extraFormats: Record<
  string,
  Record<string, string> & { [DefaultCase]?: string }
> = {
  payerCreationSource: {
  ...
  debtUpdate: {
    payerId:
      'Changed the payer of debt {link("debt", name)} from {link("from", from)} to {link("to", to)}.',
    centerId:
      'Changed the collection of debt {link("debt", name)} from {link("from", from)} to {link("to", to)}.',
    dueDate:
      'Changed the due date of debt {link("debt", name)} from {date(from)} to {date(to)}.',
    [DefaultCase]:
      'Update field {lookup("debtFields", field)} of debt {link("debt")} from value {from} to {to}.',
  },
};

const formats: Record<AuditEventAction, string> = {
  'debt.create':
    'Created a debt {link("object")} for payer {link("debtor")} with a total of {cents(total)}.',
  'debt.publish':
    'Published a debt {link("debt", name)} of {cents(total)} for payer {link("payer")}.',
  'debt.credit':
    'Credited a debt {link("debt")} for payer {link("payer")} of amount {cents(total)}.',
  'debt.update.add-component':
    'Added component "{componentName}" ({cents(componentAmount)}) to debt {link("debt")}.',
  'debt.update.remove-component':
    'Removed component "{componentName}" ({cents(componentAmount)}) from debt {link("debt")}.',
  'debt.update': '{lookup("debtUpdate", field)}',
  'debt.delete':
    'Deleted a debt named {name} of {cents(total)} for payer {link("payer", payer)}.',
  'payer.merge': 'Merged payer profile {link("from")} to {link("object")}.',
  'payer.update':
    'Updated field {field} of payer {link("object")} from value {value(oldValue)} to {value(newValue)}.',
  ...
  'bank-statement.create':
    'Imported {link("statement", "a bank statement")} for time period {date(start)} - {date(end)}.',
  'payer.create':
    "Created a payer profile {link('object')} based on {lookup('payerCreationSource', source)}.",
};
      ...
      <InfiniteTable
        ...
        columns={[
          ...
          },
          {
            ...
            render: value => value && <ResourceLink type="payer" id={value} />,
          },
          {
            name: 'Action',
            key: 'type',
                ...
                <ErrorBoundary
                  fallback={
                    <span className="italic text-gray-500">
                      Error while generating action description.
                    </span>
                  }
                >
