...
import { useGetPaymentByIntentQuery } from '../../api/stripe';
...
export const StripePaymentReturnPage = (props: Props) => {
  ...
  const intent = useMemo(
    () => new URLSearchParams(search).get('payment_intent'),
    [search],
  );
  ...
  const {
    ...
    refetch,
  } = useGetPaymentByIntentQuery(intent ?? skipToken);
  const { data: debts } = useGetDebtsByPaymentQuery(
    payment?.payment?.id ?? skipToken,
  );
  ...
  useEffect(() => {
    if (!isPaymentLoading && payment && payment.payment.status === 'unpaid') {
  ...
  if (!payment || payment.payment.status === 'unpaid') {
    return (
