...
const UserLink = ({ id }: { id: InternalIdentity }) => {
  ...
  return <Link onClick={(e) => e.stopPropagation()} to={`/admin/payers/${id.value}`} className="flex gap-1 items-center">{user?.name} <ExternalLink className="text-blue-500 h-4" /></Link>;
};
    ...
    <>
      ...
      <TableView
        ...
        columns={[
          {
            name: 'Identifier',
            getValue: (report) => report.revision > 1 ? `${report.humanId} Rev. ${report.revision}` : report.humanId,
          },
          {
            name: 'Name',
            getValue: 'name',
          ...
          },
          {
            ...
            render: (generatedBy) => generatedBy ? <UserLink id={generatedBy} /> : 'Unknown', // format(parseISO(generatedAt), 'dd.MM.yyyy HH:mm'),
          },
          {
            name: 'Status',
            getValue: 'status',
            render: (status) => <div className="flex gap-1 items-center">
              {{
                'generating': <Loader className="text-blue-600 h-4 animate-[spin_3s_linear_infinite]" />,
