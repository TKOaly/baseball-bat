...
export const Notification = (props: Props) => {
  ...
  const notification = useAppSelector(state =>
    selectNotification(state, props.id),
  );
  ...
  return (
    <div
      className={baseClasses({ type: notification.type })}
    ...
    >
      ...
      <div className="col-span-full">
        <div
          style={{ animation: `${dismissDuration}ms linear 1 progress` }}
          className={timerClasses({ type: notification.type })}
        ></div>
      </div>
    </div>
  );
